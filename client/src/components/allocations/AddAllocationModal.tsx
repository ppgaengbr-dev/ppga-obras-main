...
interface AddAllocationModalProps {
  ...
  works: Work[];
  providers: Provider[];
  onAddAllocation: (data: any) => Promise<void>;
}
...
export default function AddAllocationModal({
  ...
  // Atualizar dados do prestador quando selecionado
  useEffect(() => {
    if (providerIdRef.current) {
      ...
      // Preencher campos com dados do prestador
      if (provider) {
        if (categoryRef.current) categoryRef.current.value = provider.category || '';
        if (observationRef.current) observationRef.current.value = provider.observation || '';
        if (remunerationRef.current) remunerationRef.current.value = provider.remuneration || '';
        if (baseValueRef.current) baseValueRef.current.value = provider.baseValue || '';
      }
    }
  }, [isOpen, providers]);
  ...
  const handleSave = async () => {
    ...
    try {
      console.log('[DEBUG] Enviando dados:', { workId, providerId, startDate, endDate, category, observation, remuneration, baseValue });
      ...
      if (providerIdRef.current) providerIdRef.current.value = '';
      if (startDateRef.current) startDateRef.current.value = '';
      if (endDateRef.current) endDateRef.current.value = '';
      if (categoryRef.current) categoryRef.current.value = '';
      if (observationRef.current) observationRef.current.value = '';
      if (remunerationRef.current) remunerationRef.current.value = '';
      if (baseValueRef.current) baseValueRef.current.value = '';
